...
class AdvancedLogger:
    ...
    def log_trade_event(self, trade_data: Dict[str, Any]):
        ...
        if self.config.enable_trade_monitoring:
            self.logger.info(f"TRADE_EVENT: {json.dumps(trade_data)}")
    ...
    def log_strategy_event(self, strategy_name: str, event_type: str, data: Dict[str, Any]):
        ...
        self.logger.info(f"STRATEGY_EVENT: {json.dumps(event_data)}")
...
class HealthChecker:
    ...
    def _check_strategy_health(self) -> Dict[str, Any]:
        ...
        try:
            # Esta es una verificación básica - expandir según necesidades
            return {
                'healthy': True,
                'message': 'Strategy health check passed',
                'active_strategies': 0  # Se actualizaría con estrategias reales
            }
        except Exception as e:
