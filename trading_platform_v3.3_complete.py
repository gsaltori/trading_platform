#!/usr/bin/env python3
"""
Trading Platform v3.3 - GUI Completa con Backtesting Individual
Características:
- Generación automática de estrategias
- Backtesting individual de estrategias guardadas
- Gestión completa de estrategias (guardar/cargar/eliminar)
...
class ImprovedRSIStrategy(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(1, len(data)):
            ...
            rsi_cross_up = (data['rsi'].iloc[i-1] < oversold and data['rsi'].iloc[i] >= oversold)
            rsi_cross_down = (data['rsi'].iloc[i-1] > overbought and data['rsi'].iloc[i] <= overbought)
            
            adx_ok = data['adx'].iloc[i] > 15
            
            if rsi_cross_up and adx_ok:
                if self.config.direction_bias in ['both', 'long']:
                    data.loc[data.index[i], 'signal'] = 1
            elif rsi_cross_down and adx_ok:
                if self.config.direction_bias in ['both', 'short']:
                    data.loc[data.index[i], 'signal'] = -1
...
class ImprovedMACDStrategy(BaseStrategy):
    def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if params.get('use_ma_200', False):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        params = self.config.parameters
        use_ma_200 = params.get('use_ma_200', False)
        ...
        for i in range(1, len(data)):
            ...
            ma_200_ok = True
            if use_ma_200 and not pd.isna(data.get('ma_200', pd.Series([np.nan])).iloc[i]):
                ma_200_ok = (data['close'].iloc[i] > data['ma_200'].iloc[i] if macd_cross_up else
                            data['close'].iloc[i] < data['ma_200'].iloc[i] if macd_cross_down else True)
            
            if macd_cross_up and adx_ok and ma_200_ok:
                if self.config.direction_bias in ['both', 'long']:
                    data.loc[data.index[i], 'signal'] = 1
            elif macd_cross_down and adx_ok and ma_200_ok:
                if self.config.direction_bias in ['both', 'short']:
                    data.loc[data.index[i], 'signal'] = -1
...
class BollingerBandsStrategy(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(1, len(data)):
            if pd.isna(data['bb_position'].iloc[i]):
                continue
            
            oversold_bb = data['bb_position'].iloc[i] < 0.2 and data['rsi'].iloc[i] < 35
            overbought_bb = data['bb_position'].iloc[i] > 0.8 and data['rsi'].iloc[i] > 65
...
class ATRBreakoutStrategy(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(1, len(data)):
            if pd.isna(data['highest_high'].iloc[i]) or pd.isna(data['adx'].iloc[i]):
                continue
            
            breakout_up = data['close'].iloc[i] > data['highest_high'].iloc[i-1]
            breakout_down = data['close'].iloc[i] < data['lowest_low'].iloc[i-1]
            adx_ok = data['adx'].iloc[i] > 20
...
class TradingPlatformGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("📊 Trading Platform v3.3 - Completa con Backtesting")
    ...
    def setup_gui(self):
        ...
        self.notebook.add(self.tab1, text="🔧 Generar Estrategias")
        self.notebook.add(self.tab2, text="📊 Backtesting Individual")
        self.notebook.add(self.tab3, text="💾 Estrategias Guardadas")
    ...
    def setup_tab1(self):
        ...
        self.timeframe_combo = ttk.Combobox(row1, values=['M1','M5','M15','M30','H1','H4','D1'], width=8)
        ...
        ttk.Label(row2, text="Sharpe mín:").pack(side=tk.LEFT, padx=5)
        self.sharpe_min_spin = ttk.Spinbox(row2, from_=-5, to=5, increment=0.1, width=8)
        self.sharpe_min_spin.set(0.0)
        self.sharpe_min_spin.pack(side=tk.LEFT, padx=5)
        
        ttk.Label(row2, text="WR mín %:").pack(side=tk.LEFT, padx=5)
        self.wr_min_spin = ttk.Spinbox(row2, from_=0, to=100, width=8)
    ...
    def setup_tab2(self):
        ...
        self.bt_symbol_combo = ttk.Combobox(row1, values=['EURUSD','GBPUSD','USDJPY','AUDUSD','USDCAD','NZDUSD','XAUUSD','XAGUSD'], width=12)
        self.bt_symbol_combo.set('EURUSD')
        self.bt_symbol_combo.pack(side=tk.LEFT, padx=5)
        
        ttk.Label(row1, text="Timeframe:").pack(side=tk.LEFT, padx=5)
        self.bt_timeframe_combo = ttk.Combobox(row1, values=['M1','M5','M15','M30','H1','H4','D1'], width=8)
        self.bt_timeframe_combo.set('D1')
        self.bt_timeframe_combo.pack(side=tk.LEFT, padx=5)
        
        ttk.Label(row1, text="Días:").pack(side=tk.LEFT, padx=5)
        self.bt_days_spin = ttk.Spinbox(row1, from_=30, to=3650, width=8)
        self.bt_days_spin.set(1825)
        self.bt_days_spin.pack(side=tk.LEFT, padx=5)
        
        ttk.Button(row1, text="▶️ Ejecutar Backtest", command=self.run_individual_backtest, 
                  style='Accent.TButton').pack(side=tk.LEFT, padx=20)
        
        # Resultados
        results_frame = ttk.LabelFrame(self.tab2, text="📊 Resultados", padding=10)
        results_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # Text área para resultados
    ...
    def setup_tab3(self):
        ...
        ttk.Button(toolbar, text="📤 Exportar Seleccionada", command=self.export_selected_strategy).pack(side=tk.LEFT, padx=5)
        ttk.Button(toolbar, text="🗑️ Eliminar Seleccionada", command=self.delete_selected_strategy).pack(side=tk.LEFT, padx=5)
        
        # Tabla de estrategias
        table_frame = ttk.Frame(self.tab3)
        table_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        columns = ('Nombre', 'Símbolo', 'Tipo', 'Sesgo', 'Sharpe', 'WR', 'Fecha')
        ...
        for col in columns:
            ...
            width = 250 if col == 'Nombre' else 100
    ...
    def generate_strategies(self):
        ...
        self.log("ℹ️ Iniciando generación...")
        ...
        strategies = self.generate_random_strategies(num_strategies, list(data_dict.keys()), direction_bias)
        ...
        for i, strategy in enumerate(strategies, 1):
            try:
                ...
                if result.sharpe_ratio >= sharpe_min and result.win_rate >= wr_min and result.total_trades >= 5:
                    ...
                    self.log(f"ℹ️ ✗ {strategy.name}: Filtrado (Sharpe {result.sharpe_ratio:.2f}, WR {result.win_rate:.1f}%)")
    ...
    def download_data(self, symbol: str, timeframe: str, days: int) -> pd.DataFrame:
        ...
        rates = mt5.copy_rates_from_pos(symbol, tf_map[timeframe], 0, days)
        if rates is None:
            return None
    ...
    def generate_random_strategies(self, num: int, symbols: List[str], bias: str) -> List[BaseStrategy]:
        strategies = []
        ...
        for i in range(num):
            ...
            if strategy_type == 'ma_crossover':
                ...
                config = StrategyConfig(
                    name=f"Strat_{strategy_type}_{bias}_{i+1}_{symbol}",
                    symbol=symbol,
                ...
                strategies.append(ImprovedRSIStrategy(config))
                ...
                params = {
                    ...
                    'use_ma_200': random.choice([True, False])
                }
                config = StrategyConfig(
                    name=f"Strat_{strategy_type}_{bias}_{i+1}_{symbol}",
                    symbol=symbol,
                ...
                strategies.append(BollingerBandsStrategy(config))
            
            elif strategy_type == 'atr_breakout':
                params = {
                    'atr_period': random.randint(10, 20),
                    'lookback_period': random.randint(15, 30)
                }
                config = StrategyConfig(
                    name=f"Strat_{strategy_type}_{bias}_{i+1}_{symbol}",
                    symbol=symbol,
                ...
                strategies.append(ATRBreakoutStrategy(config))
        
        return strategies
    
    def save_viable_strategies(self):
        if not self.current_results:
            messagebox.showwarning("Advertencia", "No hay estrategias viables para guardar")
            return
        ...
        for strategy, result in self.current_results:
            try:
                ...
                self.log(f"❌ Error guardando {strategy.name}: {e}")
        
        self.log(f"✅ Guardadas {saved_count} estrategias")
        self.load_saved_strategies()
        messagebox.showinfo("Éxito", f"Se guardaron {saved_count} estrategias")
            ...
            try:
                with open(filename, 'rb') as f:
                    ...
                    self.strategies_tree.insert('', 'end', values=(
                        ...
                        config.get('direction_bias', 'both'),
                        f"{result.get('sharpe_ratio', 0):.2f}",
                        f"{result.get('win_rate', 0):.1f}%",
                        config.get('created_date', 'N/A')
                    ))
    ...
    def run_individual_backtest(self):
        ...
        if not strategy_data:
            messagebox.showerror("Error", "Estrategia no encontrada")
            return
        
        # Obtener configuración backtest
        ...
        try:
            ...
            if data is None or len(data) < 100:
                raise Exception("No se pudieron descargar datos suficientes")
            
            # Crear estrategia
            config = StrategyConfig(**strategy_data['config'])
            config.symbol = symbol  # Override symbol
            
            strategy_type = config.strategy_type
            if strategy_type == 'ma_crossover':
                strategy = ImprovedMAStrategy(config)
            elif strategy_type == 'rsi':
                strategy = ImprovedRSIStrategy(config)
            elif strategy_type == 'macd':
                ...
                raise Exception(f"Tipo de estrategia no soportado: {strategy_type}")
            ...
            messagebox.showerror("Error", f"Error en backtest:\n{e}")
...
Trades Ganadores:{result.winning_trades:>10}
Trades Perdedores:{result.losing_trades:>10}
...
Fecha creación:  {config.get('created_date', 'N/A')}

MÉTRICAS DEL BACKTEST ORIGINAL:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Sharpe Ratio:    {result.get('sharpe_ratio', 0):.2f}
Win Rate:        {result.get('win_rate', 0):.1f}%
Retorno Total:   {result.get('total_return', 0):.2f}%
...
"""
            ...
            try:
                ...
                for strategy, result in self.current_results:
                    rows.append({
                        ...
                        'RetornoTotal': result.total_return,
                ...
                self.log(f"✅ Exportado a {filename}")
    ...
    def import_strategy(self):
        ...
        if filename:
            try:
                ...
                self.load_saved_strategies()
                messagebox.showinfo("Éxito", "Estrategia importada correctamente")
            except Exception as e:
    ...
    def export_selected_strategy(self):
        ...
        if filename:
            try:
                ...
                messagebox.showinfo("Éxito", f"Estrategia exportada a:\n{filename}")
            except Exception as e:
                messagebox.showerror("Error", f"Error exportando:\n{e}")
    ...
    def delete_selected_strategy(self):
        ...
        if messagebox.askyesno("Confirmar", f"¿Eliminar estrategia?\n{strategy_name}"):
            strategy_data = next((s for s in self.saved_strategies if s['config']['name'] == strategy_name), None)
            if strategy_data:
                try:
                    strategy_data['filename'].unlink()
                    self.load_saved_strategies()
                    messagebox.showinfo("Éxito", "Estrategia eliminada")
                except Exception as e:
